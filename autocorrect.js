 ...
 */
const getRelevantSentences = (targetKeyword, sentencesArray) => {
  return sentencesArray.filter((sentence) => sentence.includes(targetKeyword));
};
/**
 ...
 */
const getDistanceByLength = (length1, length2) => {
 ...
 */
const getClosestWord = (target, wordsArray) => {
  let closestWord = "";
  let minDifference = Infinity;

  wordsArray.forEach((word) => {
    const difference = Math.abs(word - target);
 ...
 */
const getClosestWordByLength = (targetLength, wordsArray) => {
  let closestWord = "";
  let minDifference = Infinity;

  wordsArray.forEach((word) => {
    const difference = Math.abs(word.length - targetLength);
    if (difference < minDifference) {
      minDifference = difference;
      closestWord = word;
    }
